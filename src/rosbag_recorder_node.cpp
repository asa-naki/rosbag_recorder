...
void RosbagRecorderNode::split_bag_file()
{
  ...
  record_options.include_hidden_topics = config_.include_hidden_topics;
...
#ifdef ROS_DISTRO_JAZZY
  record_options.topics.assign(topics_to_record.begin(), topics_to_record.end());
  
  // Add services if enabled
  if (config_.record_services) {
    for (const auto & service : config_.services) {
      record_options.services.push_back(service);
    }
  }
#endif
  
  // Open the bag with converter options
#ifdef ROS_DISTRO_JAZZY
  rosbag2_cpp::ConverterOptions converter_options;
  converter_options.input_serialization_format = config_.serialization_format;
  converter_options.output_serialization_format = config_.serialization_format;
  writer_->open(storage_options, converter_options);
#elif defined(ROS_DISTRO_HUMBLE)
  // For Humble, use simpler API - just open with storage options
...
#ifdef ROS_DISTRO_JAZZY
  RCLCPP_INFO(this->get_logger(), "Recording %zu topics and %zu services", 
              topics_to_record.size(), config_.services.size());
#elif defined(ROS_DISTRO_HUMBLE)
